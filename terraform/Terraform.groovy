...
def void initAndApply(String path){
	dir("$WORKSPACE/$path"){
	   ...
	   // withAWS(credentials: 'aws_admin', region: 'us-east-1') {
    		...
    		sh "docker run -w /app -v /root/.aws:/root/.aws -v `pwd`:/app hashicorp/terraform:light apply -auto-approve -var=\"AWS_ACCESS_KEY=$AWS_ACCESS_KEY_ID\" -var=\"AWS_SECRET_KEY=$AWS_SECRET_ACCESS_KEY\" -var=\"app_image=705110607627.dkr.ecr.us-east-1.amazonaws.com/anagrams:${BUILD_NUMBER}\""
